...
Macoestro exposes **49 MCP tools**. This file is generated from the live server's `tools/list` response (v1.3.0), so descriptions match exactly what an agent sees.
...
Check if Macoestro has the required macOS permissions (Accessibility and Screen Recording)
