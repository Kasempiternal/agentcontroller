...
internal static class ElementTools
{
    internal static void Register(ToolRegistry registry, UiAutomationService automation)
    {
        ...
        registry.Register("describe_screen", "Alias of snapshot for cross-platform AgentController compatibility.", snapshotSchema,
            args => ToolResult.Json(automation.Snapshot(AppTools.Required(args, "app"), Depth(args), MaxElements(args), Window(args))), readOnly: true);
        ...
        registry.Register("find_elements", "Find UI Automation elements using AgentController-compatible selectors.", ToolSchema.Object(ToolSchema.SelectorProperties(), "app"), args =>
        {
