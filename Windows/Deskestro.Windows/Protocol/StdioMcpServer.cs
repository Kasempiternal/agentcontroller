...
internal sealed class StdioMcpServer(ToolRegistry registry)
{
    ...
    public void Run()
    {
        ...
        while ((line = Console.ReadLine()) is not null)
        {
            ...
            {
                Console.Error.WriteLine($"deskestro-windows request error: {ex}");
    ...
    private static JsonObject Initialize(JsonObject? parameters)
    {
        ...
        return new JsonObject
        {
            ...
            {
                ["name"] = "deskestro-windows",
                ["version"] = "2.0.0"
            },
            ["instructions"] =
                "Deskestro Windows drives desktop applications through Windows UI Automation. " +
    ...
    private JsonObject CallTool(JsonObject? parameters)
    {
        ...
        {
            Console.Error.WriteLine($"deskestro-windows tool {name} failed: {ex}");
            return ToolResult.Error($"{name}: {ex.Message}");
        }
    }
