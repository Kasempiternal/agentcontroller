...
internal static class FlowTools
{
    private static readonly string FlowDirectory = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Deskestro", "flows");
    ...
    internal static void Register(ToolRegistry registry)
    {
        ...
        registry.Register("run_steps", "Run a sequence of Deskestro tool calls in order.", ToolSchema.Object(runProperties, "steps"),
        ...
        registry.Register("list_flows", "List saved Windows Deskestro flows.", ToolSchema.Empty(), _ =>
        {
            Directory.CreateDirectory(FlowDirectory);
        ...
        registry.Register("run_saved_flow", "Run a previously saved Windows Deskestro flow.", ToolSchema.Object(runSavedProperties, "name"), args =>
        {
            var name = Sanitize(AppTools.Required(args, "name"));
            var path = Path.Combine(FlowDirectory, name + ".json");
