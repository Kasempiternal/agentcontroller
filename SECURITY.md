...
Please include the affected platform (macOS, Windows, Linux, or iOS), the version from the
...
- On Linux it reads AT-SPI trees, captures the screen with ImageMagick `import`
  or grim, and can synthesize pointer/keyboard input when `foreground:true` is
  set.
...
|---|---|
| Network reachability | The macOS HTTP server binds **IPv4 loopback only**, never `0.0.0.0`. The Windows and Linux backends open **no listening socket**. |
...
| Child processes | Linux (and iOS) spawn helpers with **argument arrays**, never interpolated shell strings. |

The Windows and Linux backends speak MCP over stdio and open **no listening
socket**, so the loopback and token considerations above are macOS-specific.
...
- Bypassing macOS TCC, Windows UIPI, or Linux AT-SPI/compositor permissions is
  out of scope; AgentController relies on those grants rather than circumventing them.
