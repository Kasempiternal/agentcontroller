...
class AtspiService:
    ...
    def list_actions(self, element: Any) -> list[dict[str, str]]:
        ...
        desc_fn = getattr(action, "get_action_description", None) or getattr(action, "get_description", None)
        actions: list[dict[str, str]] = []
        for index in range(count):
            name = ""
            if callable(name_fn):
                try:
                    name = str(name_fn(index) or "")
                except Exception:
                    name = ""
            does = ""
            if callable(desc_fn):
                try:
                    does = str(desc_fn(index) or "")
                except Exception:
                    does = ""
            actions.append({"name": name, "does": does or name})
        return actions

    def perform_named_action(self, element: Any, name: str) -> bool:
        action = _iface(element, "get_action_iface", "get_action", "queryAction")
        if action is None:
            return False
        count_fn = getattr(action, "get_n_actions", None)
