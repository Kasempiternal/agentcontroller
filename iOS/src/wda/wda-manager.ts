...
export class WDAManager {
  ...
  private getWdaProjectPath(): string {
    for (const root of runtimeRoots()) {
      const candidate = join(root, 'wda-build', 'WebDriverAgent')
      if (existsSync(join(candidate, 'WebDriverAgent.xcodeproj'))) return candidate
    }
    throw new Error('WebDriverAgent not found. Run the iOS backend CLI with --setup to install it.')
  }
  ...
  private async ensureWdaSource(): Promise<string> {
    try {
      ...
      const targetDir = join(RUNTIME_HOME, 'wda-build')
  ...
  private getDerivedDataPath(): string {
    // If a previous install already built WDA under an old root, keep using it
    // so derived data (and the installed provisioning) is not rebuilt.
    for (const root of runtimeRoots()) {
      const dir = join(root, 'wda-build')
      if (existsSync(dir)) return dir
    }
    const dir = join(RUNTIME_HOME, 'wda-build')
  ...
  private async preTriggerCodesignDialog(teamId: string): Promise<void> {
    try {
      ...
      const tmpBin = `/tmp/agentcontroller_codesign_preflight_${Date.now()}`
